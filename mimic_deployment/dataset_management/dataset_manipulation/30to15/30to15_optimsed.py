...
def main(orig_repo_id, new_repo_id):
    ...
    # --- 2. LOAD AND CREATE ---
    print(f"\n--- Loading original dataset: {orig_repo_id} ---", flush=True)
    ...
    print("\nFinalizing dataset metadata...", flush=True)
    ...
    print(f"Upload complete for {new_repo_id}!\n", flush=True)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert 30hz LeRobot dataset to 15hz.")
    parser.add_argument("--orig", required=True, help="Original dataset repo ID")
    parser.add_argument("--dest", required=True, help="Destination dataset repo ID")
    
    args = parser.parse_args()
    main(args.orig, args.dest)