...
def udp_listener_thread():
    """This thread listens for incoming UDP IDs and updates the task."""
    global current_task
    
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind((UDP_IP, UDP_PORT))
    print(f"\n[Network] Listening for UDP commands on port {UDP_PORT}...")
    
    while True:
        # This will block until a packet is received, without stopping the main robot loop
        ...
        if move in TIC_TAC_TOE_MOVES:
            ...
            print(f"\n> [UDP] Received Move ID {move}. Instructing Smol-VLA: {current_task}")
